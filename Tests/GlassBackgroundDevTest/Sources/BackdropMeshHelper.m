...
static void calculateDistortionOptimized(CGFloat x, CGFloat y,
                                         CGFloat distortionStrength,
                                         CGRect bounds, CGFloat cornerRadius,
  ...
  // Falloff / Intensity
  // Note: 4.0 factor from original code kept for consistency
  // CGFloat intensity = 1.0 - fabs(4.0 * d);
  CGFloat intensity = 1.3 - fabs(4.0 * d);
  intensity = fmax(intensity, 0.0);
  // Replace pow(x, 2) with x*x for speed
...
+ (CALayer *)createBackdropLayerWithBlurRadius:(CGFloat)blurRadius
  ...
  // Optimization: Capture at reduced scale. Since we blur the content,
  // Optimization: Capture at reduced scale.
  // 0.5 (1/2 scale) strikes a better balance for stronger blurs than 0.25,
  // preventing pixelation while still saving memory.
  backdropLayer.scale = 0.5;
...
+ (id)createOptimizedLensDistortionMeshWithConfiguration:
    (NSDictionary *)config {
  CGFloat distortionStrength = [config[@"distortionStrength"] floatValue];
  CGRect bounds = [config[@"bounds"] CGRectValue];
  CGFloat cornerRadius = [config[@"cornerRadius"] floatValue];

  // 1. Caching Check
  NSString *cacheKey = [NSString
      stringWithFormat:@"%.2f-%.1f-%.1f-%.1f", distortionStrength,
                       bounds.size.width, bounds.size.height, cornerRadius];
  ...
  for (NSInteger r = 0; r < rows; r++) {
    ...
    for (NSInteger c = 0; c < cols; c++) {
      ...
      calculateDistortionOptimized(x, y, distortionStrength, bounds,
                                   cornerRadius, &distX, &distY, &distZ);
